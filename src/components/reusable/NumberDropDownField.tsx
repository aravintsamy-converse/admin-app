  ...
  return (
    <>
      <div
        ...
        className={`${hidden ? 'hidden' : ''} flex items-end 3xl:w-[508px] group relative  `}
      >
        <FormField
          control={control}
          ...
          render={({ field, fieldState }) => (
            <FormItem className=" !w-full  ">
              <FormLabel
                className={`${hidden ? 'hidden' : ''} font-normal opacity-95 text-foreground text-[15px] whitespace-nowrap`}
              >
                {label}
              ...
              <FormControl className={`${hidden ? 'hidden' : ''}`}>
                <Input
                  ...
                  className={`transition-transform group-hover:border-primary shadow-none group-hover:!border-r-0  ${focusID || open ? '!border-primary !border-r-0' : ''} !mt-[6px]   h-[35px] w-full text-selectedValue font-normal border border-border  !rounded-r-none  border-r-0 rounded-sm focus:!outline-none focus:!ring-0  focus-within:!ring-0 focus-within:!outline-none `}
                />
              </FormControl>
          ...
          render={({ field, fieldState }) => (
            <FormItem>
              <FormControl>
                ...
                >
                  <SelectTrigger
                    className={`group-hover:border-primary pl-0 flex  justify-around  ${focusID || open ? '!border-primary' : ''} w-[100px] !mt-[6px] h-[35px] rounded-sm font-normal text-selectedValue border border-border border-l-0 rounded-l-none  focus:!outline-none  focus:!ring-0  focus-within:!ring-0 focus-within:!outline-none `}
                  >
                    <SelectValue />
                    <ChevronDown
